...
namespace stdexec {
  ...
  namespace __get_completion_signatures {
    ...
    concept __with_tag_invoke = __valid<
      tag_invoke_result_t,
    ...
    template <class _Sender, class _Env>
    using __member_alias_t = typename std::remove_reference_t<
      sender_transform_result_t<_Sender, _Env>>::completion_signatures;
  ...
  extern const __connect::connect_t connect;
  ...
  namespace __connect {
    ...
    concept __connectable = //
      __connectable_<__tfx_sender<_Sender, _Receiver>, _Receiver>;

    struct connect_t {
      // Dispatch to a tag_invoke customization for connect:
      template <class _Sender, class _Receiver>
        requires __connectable_with_tag_invoke<_Sender, _Receiver>
      static auto __connect_(_Sender&& __sndr, _Receiver&& __rcvr) //
        ...
        return tag_invoke(connect, (_Sender&&) __sndr, (_Receiver&&) __rcvr);
      }

      // The sender is an awaitable, so connect by awaiting it in a coroutine:
      template <class _Sender, class _Receiver>
        requires __connectable_with_tag_invoke<_Sender, _Receiver> || //
                 __callable<__connect_awaitable_t, _Sender, _Receiver>
      static auto __connect_(_Sender&& __sndr, _Receiver&& __rcvr)
      ...
      // The type isn't connectable at all but the user has asked for a backtrace:
      template <class _Sender, class _Receiver>
        requires __connectable_with_tag_invoke<_Sender, _Receiver> ||     //
                 __callable<__connect_awaitable_t, _Sender, _Receiver> || //
                 tag_invocable<__is_debug_env_t, env_of_t<_Receiver>>
      static auto __connect_(_Sender&& __sndr, _Receiver&& __rcvr) {
        ...
        tag_invoke(connect, (_Sender&&) __sndr, (_Receiver&&) __rcvr);
      }

      template <class _Sender, class _Receiver>
      static constexpr bool __nothrow_connect = //
        noexcept(__connect_(
          sender_transform(__declval<_Sender>(), get_env(__declval<_Receiver&>())),
          __declval<_Receiver>()));

      template <class _Sender, class _Receiver>
      using __connect_result_t = //
        decltype(__connect_(
          sender_transform(__declval<_Sender>(), get_env(__declval<_Receiver&>())),
          __declval<_Receiver>()));
      ...
      auto operator()(_Sender&& __sndr, _Receiver&& __rcvr) const //
        ...
        -> __connect_result_t<_Sender, _Receiver> {
        ...
        return __connect_((_Sender&&) __sndr, (_Receiver&&) __rcvr);
      }
  ...
  namespace __just {
    ...
    inline constexpr struct __just_t {
      ...
      STDEXEC_DETAIL_CUDACC_HOST_DEVICE //
        ...
        operator()(_Ts&&... __ts) const noexcept((__nothrow_movable_value<_Ts> && ...)) {
        return {{{(_Ts&&) __ts...}}};
      }
    } just{};
    ...
    inline constexpr struct __just_error_t {
      ...
      STDEXEC_DETAIL_CUDACC_HOST_DEVICE //
        ...
        operator()(_Error&& __err) const noexcept(__nothrow_movable_value<_Error>) {
        return {{{(_Error&&) __err}}};
      }
    } just_error{};
    ...
    struct __stopped_sender : __t<__basic_sender<__just_stopped_t>> {
      using __id = __stopped_sender;
      using __t = __stopped_sender;
    };

    STDEXEC_DETAIL_CUDACC_HOST_DEVICE //
      inline __stopped_sender
      __just_stopped_t::operator()() const noexcept {
      return {{}};
    }
  }
    ...
    struct __sender {
      ...
      struct __t {
        ...
        template <
          ...
          class _Init,
          class _Reduce,
          __callable<_Sender, _Init, _Reduce> _Transform>
          requires __callable<
            _Reduce,
            then_t,
            _Init,
        ...
        friend auto sender_transform_reduce(
          _Self&& __self,
          _Init&& __init,
          _Reduce&& __reduce,
          ...
          return ((_Reduce&&) __reduce)(
            ...
            ((_Self&&) __self).__fun_,
            ((_Transform&&) __transform)(
              ((_Self&&) __self).__sndr_, (_Init&&) __init, (_Reduce&&) __reduce));
        }
  ...
  namespace __sync_wait {
    ...
    struct sync_wait_t {
      ...
      template <__single_value_variant_sender<__env> _Sender> // NOT TO SPEC
        requires __tag_invocable_with_domain< sync_wait_t, set_value_t, _Sender>
      tag_invoke_result_t< sync_wait_t, __sender_domain_t<_Sender, set_value_t>, _Sender>
        operator()(_Sender&& __sndr) const noexcept(
          nothrow_tag_invocable< sync_wait_t, __sender_domain_t<_Sender, set_value_t>, _Sender>) {
